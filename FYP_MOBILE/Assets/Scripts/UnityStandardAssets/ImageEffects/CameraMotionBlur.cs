	...
	public class CameraMotionBlur : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (filterType == MotionBlurFilter.Reconstruction || flag || filterType == MotionBlurFilter.ReconstructionDisc)
			{
				...
				num2 = divRoundUp(temporary.width, (int)maxVelocity);
				height = divRoundUp(temporary.height, (int)maxVelocity);
				num = temporary.width / num2;
			}
			else
			{
				num2 = divRoundUp(temporary.width, (int)maxVelocity);
			...
			RenderTexture temporary2 = RenderTexture.GetTemporary(num2, height, 0, format);
			RenderTexture temporary3 = RenderTexture.GetTemporary(num2, height, 0, format);
			...
			motionBlurMaterial.SetFloat("_MaxVelocity", num);
			motionBlurMaterial.SetFloat("_MaxRadiusOrKInPaper", num);
			...
			if (filterType == MotionBlurFilter.CameraMotion)
			{
				...
				float num3 = Vector3.Dot(base.transform.up, Vector3.up);
				Vector3 rhs = prevFramePos - base.transform.position;
				float magnitude = rhs.magnitude;
				float num4 = Vector3.Angle(base.transform.up, prevFrameUp) / _camera.fieldOfView * ((float)source.width * 0.75f);
				zero.x = rotationScale * num4;
				num4 = Vector3.Angle(base.transform.forward, prevFrameForward) / _camera.fieldOfView * ((float)source.width * 0.75f);
				zero.y = rotationScale * num3 * num4;
				num4 = Vector3.Angle(base.transform.forward, prevFrameForward) / _camera.fieldOfView * ((float)source.width * 0.75f);
				zero.z = rotationScale * (1f - num3) * num4;
			...
			else if (filterType == MotionBlurFilter.ReconstructionDX11 && !flag)
			{
				...
				dx11MotionBlurMaterial.SetFloat("_MaxRadiusOrKInPaper", num);

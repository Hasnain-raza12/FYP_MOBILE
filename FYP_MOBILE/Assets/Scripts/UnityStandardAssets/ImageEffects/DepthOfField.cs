		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			else if (blurType == BlurType.DX11 && (bool)dx11bokehMaterial)
			{
				...
				else
				{
					...
					RenderTexture temporary4 = RenderTexture.GetTemporary(renderTexture.width >> 1, renderTexture.height >> 1, 0, renderTexture.format);
					RenderTexture temporary5 = RenderTexture.GetTemporary(renderTexture.width >> 1, renderTexture.height >> 1, 0, renderTexture.format);
					Graphics.Blit(renderTexture, temporary4, dofHdrMaterial, 15);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(0f, 1.5f, 0f, 1.5f));
